...
| **사진집** | 에디트 풀 → 펼침면 시퀀스 → 사양·제작 기록. 사이트에는 나오지 않는 작업 공간입니다 |
| **전시** | 출품작 프린트 사양, 체크리스트·예산. `공개일`을 채우기 전에는 비공개입니다 |

**노출값은 손으로 입력하지 않아도 됩니다.** 렌즈·조리개·셔터·ISO 필드를 비워두면 업로드된 파일의 EXIF가 대신 표시됩니다. 필드에 값을 넣으면 그 값이 이깁니다 — EXIF를 고치는 용도입니다.
...
- **업로드 전에 반드시 메타데이터를 지우세요.** Sanity는 EXIF를 그대로 읽어 저장합니다

```bash
node scripts/scrub-location.mjs ~/export           # 폴더째
node scripts/scrub-location.mjs ~/export --dry-run # 뭐가 지워질지만 확인
```

원본은 건드리지 않고 `scrubbed/` 에 결과를 씁니다. `--in-place` 로 덮어쓸 수 있고, `--out <폴더>` 로 위치를 바꿀 수 있습니다.

| 지우는 것 | 왜 |
|---|---|
| GPS 좌표 | 거리 사진은 촬영 위치가 곧 동선입니다. 일주일치면 사는 곳이 나옵니다 |
| **바디·렌즈 시리얼** | 가장 위험합니다. 어디에 올렸든 같은 바디로 찍은 사진끼리 전부 묶입니다 — 사이트가 달라도, 활동명이 달라도 |
| MakerNote | 제조사 블롭. 시리얼과 셔터 카운트가 또 들어 있습니다 |
| Artist · Copyright · UserComment | 편집 프로그램이 OS 계정 이름으로 자동으로 채웁니다 |
| XMP · IPTC 블록 | 촬영자·도시 정보의 또 다른 사본 |
| 내장 썸네일 | 크롭해도 갱신되지 않아 **잘라낸 부분이 남아 있습니다** |

남기는 것은 바디·렌즈·노출·촬영일 — 시트에 실제로 표시하는 값들입니다.

**이미지는 재인코딩하지 않습니다.** 메타데이터 세그먼트만 다시 씁니다. 화질 손실이 없다는 뜻이고, 픽셀이 바이트 단위로 동일한 것을 확인했습니다.

> **JPEG만 처리합니다.** RAW·HEIC는 건너뜁니다. 라이트룸에서 JPEG으로 내보낸 뒤 이 스크립트를 통과시키세요. 내보낼 때 라이트룸의 메타데이터 옵션을 "저작권만" 으로 두면 일이 줄어듭니다.
